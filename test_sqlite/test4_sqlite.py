...
for row in results:
    try:
        ...
        if not(tag == "0" or tag == "1"):
            ...
            cursor.execute(f"CREATE TABLE \"{tag}\"  (value INTEGER, timestamp TEXT)")
            # Inserir os valores e os timestamps na nova tabela
            for i in range(len(values)):
                cursor.execute(f"INSERT INTO \"{tag}\" VALUE (?, ?)", (values[i], timestamps[i]))
    
    except sqlite3.OperationalError as e:
        if str(e) == f'table "{tag}" already exists':
            for i in range(len(values)):
                cursor.execute(f"INSERT INTO {tag} VALUES (?, ?)", (values[i], timestamps[i]))
        else:
            print(e)
